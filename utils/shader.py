...
class ShaderHandler:
    ...
    def __draw_2d_callback(self, context):
        if self.__user_callback_2d is None: return
        if not callable(self.__user_callback_2d): return
        try:
    ...
    def __draw_3d_callback(self, context):
        if self.__user_callback_3d is None: return
        if not callable(self.__user_callback_3d): return
        try:
...
@persistent
def unload_shader_handles(null=''):
